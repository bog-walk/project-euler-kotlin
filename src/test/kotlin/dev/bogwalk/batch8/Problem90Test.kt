...
internal class CubeDigitPairsTest {
    ...
    @ParameterizedTest(name="{0} cube with {1}^2")
    @CsvSource(
        // 1 cube
    ...
    fun `correct for all constraints`(square: Int, cube: Int, expected: Int) {
        assertEquals(expected, tool.countValidCubes(square, cube))
    }
}